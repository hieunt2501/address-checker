...
MODEL_PATH = './model/addressClassifier.pkl'
TFIDF_PATH = './model/tfidf_transformer.pkl'
...
def preprocess_message(message):
    message = re.sub(r'Khách [\w]*:\s','', message)
    message = re.sub(
        '[\:\_=\+\#\@\$\%\$\\(\)\~\@\;\'\|\<\>\]\[\"\–“”…*]', ' ', message)
    address_word = r'\b(đ[ịi]a chỉ|[dđ]c|)\s*(n[àạa]y|c[ủuũ]|[dđ][aâ]y|,))\b'
    ...
    # ---------------------- Chuẩn hóa 'k' thành 'không', 'j' thành 'gì'
    # list_token = ['không' if ele in ['k', 'ko', 'hok'] else ele for ele in list_token]
    while '' in list_token:
        list_token.remove('')
